...
fn main() {
    let mut weights1 = Array2::<f64>::from_shape_fn((20, 1), randomize);
    let mut weights2 = Array2::<f64>::from_shape_fn((20, 20), randomize);
    let mut weights3 = Array2::<f64>::from_shape_fn((1, 20), randomize);
...
//            println!();
        ...
        if i % 100 == 0 {
            ...
            let line = line::Line::new(&prediction_vec[..]).style(
                line::Style::new().colour("#DD3355").width(2f32)
            );
            let line2 = line::Line::new(&func_vec[..]).style(
                line::Style::new().colour("#33DEBC").width(2f32)
            );
